...
public:
    ...
    HttpServer(const typename asio::basic_socket_acceptor<Protocol>::endpoint_type& endpoint, ServerHandler handler)
...
protected:
    ...
    public:
        Connection(asio::basic_stream_socket<Protocol> socket, ServerHandler handler, const HttpParser& httpParser)
    ...
    protected:
        asio::basic_stream_socket<Protocol> _socket;
        ...
        void _readHeader() {
            ...
            auto data(std::make_shared<asio::streambuf>());
            data->prepare(1024);
            ...
            asio::async_read_until(
                    ...
                    *data,
                    "\r\n\r\n",
                    [self, data](const system::error_code& e, std::size_t n) {
                if (e) {
                    std::cout << "error in HttpServer::Connection#_readHeader: " << e << std::endl;
                    return;
                }

                asio::streambuf::const_buffers_type bufs = data->data();
                std::string dataAsString(asio::buffers_begin(bufs), asio::buffers_begin(bufs) + n);
        ...
        void _readBody(std::shared_ptr<asio::streambuf> data, unsigned long long size, std::shared_ptr<std::unordered_map<std::string, std::string>> headers) {
            auto self(this->shared_from_this());
            ...
            asio::async_read(_socket,
                                    *data,
                                    asio::transfer_exactly(size - data->size()),
                                    [self, data, size, headers](const system::error_code& e, std::size_t) {
                if (e) {
                ...
                asio::streambuf::const_buffers_type bufs = data->data();
                std::string dataAsString(asio::buffers_begin(bufs), asio::buffers_begin(bufs) + size);
    ...
    void _startAccept() {
        _acceptor.async_accept(_socket, [this](const system::error_code& e) {
            if (e) {
    ...
    asio::basic_socket_acceptor<Protocol> _acceptor;
    asio::basic_stream_socket<Protocol> _socket;
    const ServerHandler _handler;
    const HttpParser _httpParser;
};
